#!/home/hl796/.conda/envs/eddy/bin/python

##### Maybridge prediction by PDBbind2020-full-trained models (D3) (with ray) #####
#https://slurm.schedmd.com/job_array.html


##### Libraries #####
import os
PD = os.environ['GIBBS'] #'/home/hl796/gibbs'
TMP = os.environ['TMP'] #'/tmp'

print(PD, TMP)
print()

import sys
sys.path.append(PD+'/tools/DeepPurpose')
import DeepPurpose
...
#https://stackoverflow.com/questions/23943978/parallel-code-hangs-if-run-first-but-works-if-run-after-running-non-parallel-co
...
if __name__ == "__main__":
    ...
    SEQ_TARGET = PD+'/projects/eddy/rcsb_pdb_5JQ1.fasta'

    MODEL_DIR1 = PD+'/tools/DeepPurpose/training/regression/PDBbind2020-refined-regsm-fulldata/'
    MODEL_DIR2 = '/jobarray-RCV-'
    
    INPUT_FILE1 = PD+'/data/Maybridge/Maybridge_HitDiscover.smiles.unique.v2.csv'

    RESULT_FOLDER = './'
    OUTPUT_FILENAME0 = 'maybridge.predictions.by.D3.models.'
    ...
    PRTMOD = Parallel(n_jobs=len(MODELS),
                      prefer='threads',
                      verbose=1)(delayed(load_model)(k) for k in range(len(MODELS)))
    ...
    ray_output = [PredictionUnit.options(memory=20*1024*1024*1024).remote(r, PRTMOD) for r in range(N_REPEATS)]
