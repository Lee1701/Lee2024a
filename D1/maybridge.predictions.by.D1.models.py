...
##### Maybridge prediction by DeepPurpose-v0.1.5 BindingDB-Kd-pretrained models (D1) (with jobarray) #####
...
if __name__ == "__main__":
    ...
    MODEL_DIR = PD+'/models/pretrained_dp015/'
    INPUT_FILE1 = PD+'/data/Maybridge_HitDiscover.smiles.unique.v2.csv'

    RESULT_FOLDER = './'
    OUTPUT_FILENAME0 = 'maybridge.predictions.by.D1.models.'
    ...
    MODELS_names = [x.replace('model_', '')
                    .replace('_bindingdb', '')
                    .replace('aac', 'AAC')
                    .replace('cnn', 'CNN')
                    .replace('mpnn', 'MPNN')
                    .replace('daylight', 'Daylight')
                    .replace('morgan', 'Morgan')
    ...
    ##### Prediction #####
    y_pred_all = Parallel(n_jobs=len(MODELS_names),
                          prefer='threads',
                          verbose=1)(delayed(PredictionUnit)(r) for r in range(len(MODELS_names)))
